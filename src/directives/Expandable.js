let directive = {
  ...
  inserted: (el, binding) => {
    ...
    directive.scrollbarTransition.el = document
      .getElementById(parentId)
      .getElementsByClassName("vue-scrollbar-transition")[0];

    directive.scrollbarTransition.handler = event => {
      scrollbar.calculateSize();
    };

    directive.scrollbarTransition.el.addEventListener(
      "transitionend",
      directive.scrollbarTransition.handler
    );
    ...
    let expandHandler = item => {
      return event => {
        if (!item.meta.expanded) {
          scrollbar.scrollToY(
            scrollbar.top - childHeight * item.children.length
          );
        } else {
  ...
  unbind: () => {
    directive.scrollbarTransition.el.removeEventListener(
      "transitionend",
      directive.scrollbarTransition.handler
    );

    for (let i = 0; i < directive.dom.elements.length; i++) {
      directive.dom.elements[i].removeEventListener(
        "transitionend",
        directive.dom.handlers[i]
      );
    }
  }
};
