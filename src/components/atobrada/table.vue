...
import { apiUrlObradaPregled_at } from "../../../config/config.js";
  ...
  created() {
   ...
   /*  this.itemsPerPage.forEach(element => {
    ...
    http
      ...
      .then(res => {
        this.sekcije.push("Obrada uzoraka: COVID 19 Antitijela Test");
        if (!this.section.length) {
          this.section = this.sekcije[0];
        } else {
          if (this.section === "Obrada uzoraka: COVID 19 Antitijela Test") {
            this.sve = true;
          } else {
            this.sve = false;
          }
