<template>
  <div class="file-upload-list-item d-flex" :class="{'file-upload-list-item--undo': removed}">
    <div class="va-row" v-if="removed">
      <div class="flex md12">
        <vuestic-file-upload-undo @recover="recoverFile" />
      </div>
    </div>

    <div class="va-row align--center" v-else>
      <div class="file-upload-list-item__name flex xs12 lg3 ellipsis">{{ file.name }}</div>
      <div class="file-upload-list-item__size flex xs12 lg3">{{ file.size }}</div>
