              ...
              <div class="modal-header" style="padding-right: 40px;">
                <slot name="header">
                  ...
                  <i
                    ...
                    v-if="((neg || pos) && sample.status === 'ZAPRIMLJEN')"
...
export default {
  name: "vuestic-modal-micro-result",
  props: {
    transition: {
      type: String,
      default: "modal"
  ...
  methods: {
    ...
    ok() {
      this.$emit("ok");
      bus.$emit("MicroResult");
      window.removeEventListener("keyup", this.listenKeyUp);
    },
    cancel() {
      this.$emit("cancel");
      bus.$emit("CancelMicroResult");
      this.show = false;
      window.removeEventListener("keyup", this.listenKeyUp);
    },
    clickMask() {
