...
export default {
  name: "vuestic-modal-control-delete",
  props: {
    transition: {
      type: String,
      default: "modal"
  ...
  mounted() {
    bus.$on("ControlSampleDeleted", () => {
      this.show = false;
    });
  },
  beforeDestroy() {
    bus.$off("ControlSampleDeleted");
  },
  created() {
    if (this.show) {
  ...
  methods: {
    ...
    ok() {
      // console.log("Brisanje kontrolnog uzorka...")
      this.$emit("ok");
      bus.$emit("ControlSampleDelete");
      // this.show = false;
