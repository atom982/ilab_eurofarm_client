...
export default {
  name: "vuestic-data-table-control-edit",
  components: {
    FilterBar,
    Vuetable,
    VuetablePagination,
  ...
  mounted() {
    bus.$on("UređivanjeRefresh", () => {
      // console.warn("Refresh of Table Results.");
      Vue.nextTick(() => this.$refs.vuetable.refresh());
    });
  },
  beforeDestroy() {
    bus.$off("UređivanjeRefresh");
  },
  methods: {
    Akcija: function(args) {
      ...
      if (
        ...
        if (niz[1].name === "reference") {
          bus.$emit("KontroleReference", niz[0]);
        } else if (niz[1].name === "uredi") {
          bus.$emit("KontroleUredi", niz[0]);
        } else if (niz[1].name === "akcija") {
          bus.$emit("KontroleAkcija", niz[0]);
        } else if (niz[1].name === "analizator") {
          bus.$emit("KontroleAnalizator", niz[0]);
        } else {
        }
      }
    },
