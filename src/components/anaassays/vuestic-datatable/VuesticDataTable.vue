...
export default {
  name: "vuestic-data-table-anaassays",
  components: {
    FilterBar,
    Vuetable,
    VuetablePagination,
  ...
  mounted() {
    bus.$on("AnaAssayRefresh", () => {
      Vue.nextTick(() => this.$refs.vuetable.refresh());
    });
  },
  beforeDestroy() {
    bus.$off("AnaAssayRefresh");
  },
  methods: {
    Akcija: function(args) {
      ...
      if (
        ...
        if (niz[1].name === "reference") {  
          bus.$emit("AnaAssayReference", niz[0]);
        } else if (niz[1].name === "uredi") {         
          bus.$emit("AnaAssayUredi", niz[0]);
        } else if (niz[1].name === "akcija") {         
          bus.$emit("AnaAssayAkcija", niz[0]);
        } else {
        }
      }
    },
