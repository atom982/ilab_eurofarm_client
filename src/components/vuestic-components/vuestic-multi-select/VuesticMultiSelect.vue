<template>
  ...
  >
    <div class="input-group dropdown-toggle">
      <!-- @click="$refs.scrollbar.onContainerResize()"> -->
      <input readonly :class="{'has-value': !!displayValue}" v-bind:value="displayValue" required>
...
export default {
  ...
  methods: {
    toggleSelection(option) {
      let newVal = this.isOptionSelected(option)
    ...
    updateDisplayValue(newVal) {
      if (newVal.length > 2) {
        this.displayValue = `${newVal.length} od ${
          this.options.length
        } odabrana`;
      } else {
        this.displayValue = (this.optionKey
          ? newVal.map(item => item[this.optionKey])
          : newVal
        ).join(", ");
      }
    },
