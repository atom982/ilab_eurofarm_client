<template>
  <div class="form-group with-icon-right dropdown select-form-group"
    ...
    <div v-if="color === 'default'" class="input-group dropdown-toggle"> <!-- @click="$refs.scrollbar.onContainerResize()"> -->
      <input
        readonly 
    ...
    <div v-if="color != 'default'" class="input-group dropdown-toggle"> <!-- @click="$refs.scrollbar.onContainerResize()"> -->
      <input
        ...
        required/>
      <i class="ion ion-chevron-down icon-right input-icon"></i>
      <label style="color: #e34a4a" class="control-label">{{label}}</label><i class="bar"></i>
      <small v-show="hasErrors()" class="help text-danger">{{ showRequiredError() }}</small>
    </div>
