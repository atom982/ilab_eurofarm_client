            ...
            <div class="modal-content">
              ...
              <div class="modal-header" style="padding-right: 40px;">
                <slot name="header">
                  ...
                  <button
                    style="padding: 0.6px; padding-left: 3px;"
                    v-if="select && !povratak && !noButtons && sample.status != 'ODOBREN' && sample.status != 'VERIFICIRAN'"
                    @click.prevent="Update()"
                    class="btn btn-secondary-danger btn-micro"
                  >
                    <i class="fa fa-bug" @click.prevent="Update()"></i>
                  </button>
              ...
              <div class="modal-footer">
                <slot name="footer">
                  ...
                  <button
                    type="button"
                    v-if="!povratak && !noButtons && sample.status != 'ODOBREN' && sample.status != 'VERIFICIRAN'"
                    :class="okClass"
                    @click="ok"
                    :disabled="false"
                  >{{ okText }}</button>
...
export default {
  name: "vuestic-modal-micro-bactery",
  props: {
    transition: {
      type: String,
      default: "modal"
  ...
  methods: {
    ...
    ok() {
      this.$emit("ok");
      bus.$emit("MicroBactery");
      window.removeEventListener("keyup", this.listenKeyUp);
    },
    cancel() {
      this.$emit("cancel");
      bus.$emit("CancelMicroBactery");
      this.show = false;
      window.removeEventListener("keyup", this.listenKeyUp);
    },
    Povratak() {
      bus.$emit("PovratakMicroBactery");
    },
    Spasi() {
      bus.$emit("SpasiMicroBactery");
    },
    Update() {
      bus.$emit("UpdateMicroBactery");
    },
    clickMask() {
      if (!this.force) {
        this.cancel();
