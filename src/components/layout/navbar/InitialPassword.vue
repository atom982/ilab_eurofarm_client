              ...
              <div style="padding-right: 40px;" class="modal-header">
                <slot name="header">
                  ...
                  <i
                    ...
                    v-show="password"
                    class="fa fa-eye icon-right input-icon"
                  ></i>
                  <i
                    style="color: #e34a4a;"
                    @click.prevent="Password('!password')"
                    v-show="!password"
                    class="fa fa-eye-slash icon-right input-icon"
                  ></i>
                </slot>
              </div>
              <!--Container-->
...
export default {
  ...
  mounted() {
    bus.$on("ChangePasswordModalHide", () => {
      this.show = false;
    });
  },
  computed: {
  ...
  beforeDestroy() {
    bus.$off("ChangePasswordModalHide");
  ...
  methods: {
    Password(event) {
      if (event === "password") {
        bus.$emit("_ShowPasswordInit");
        this.password = false;
      } else {
        bus.$emit("_HidePasswordInit");
        this.password = true;
      }
    },
    listenKeyUp(event) {
    ...
    ok() {
      this.$emit("ok");
      bus.$emit("ChangePasswordInit");
      window.removeEventListener("keyup", this.listenKeyUp);
    },
    cancel() {
      this.$emit("cancel");
      bus.$emit("ChangePasswordCancel");
      this.password = true;
      this.show = false;
      window.removeEventListener("keyup", this.listenKeyUp);
    },
