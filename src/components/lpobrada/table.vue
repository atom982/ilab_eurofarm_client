...
import { apiUrlObradaPregled_lp } from "../../../config/config.js";
  ...
  created() {
   ...
   /*  this.itemsPerPage.forEach(element => {
    ...
    http
      ...
      .then(res => {
        this.sekcije.push("Obrada uzoraka: COVID 19 RT LAMP Test");
        if (!this.section.length) {
          this.section = this.sekcije[0];
        } else {
          if (this.section === "Obrada uzoraka: COVID 19 RT LAMP Test") {
            this.sve = true;
          } else {
            this.sve = false;
          }
